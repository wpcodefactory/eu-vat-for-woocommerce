...
if ( ! class_exists( 'WPFactory_WC_EU_VAT_Keep_VAT_Individual_Product' ) ) :

class WPFactory_WC_EU_VAT_Keep_VAT_Individual_Product {
	...
	function save_keep_vat_individual_product( $product ) {
		if ( isset( $_POST['_alg_wc_eu_vat_keep_vat'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
			$product->update_meta_data(
				'_alg_wc_eu_vat_keep_vat',
				sanitize_text_field( wp_unslash( $_POST['_alg_wc_eu_vat_keep_vat'] ) ) // phpcs:ignore WordPress.Security.NonceVerification.Missing
			);
		}
	}
...
return new WPFactory_WC_EU_VAT_Keep_VAT_Individual_Product();
