...
class WPFactory_WC_EU_VAT_Blocks_Integration implements IntegrationInterface {
	...
	public function eu_vat_country_enqueue_script() {
		...
		wp_localize_script(
			'wpfactory-wc-eu-vat-field-block-frontend',
			'wpfactory_wc_eu_vat_frontend_countries_object',
			$flipped_countries
		);
	}
	...
	public function get_script_handles() {
		return array( 'wpfactory-wc-eu-vat-field-block-frontend' );
	}
	...
	public function get_editor_script_handles() {
		return array( 'wpfactory-wc-eu-vat-field-block-editor' );
	}
	...
	public function get_script_data() {
		return array(
			'eu_vat_is_active'                           => true,
			'eu_vat_field_id'                            => '#contact-alg_eu_vat-' . wpfactory_wc_eu_vat_get_field_id(),
			'eu_vat_customer_decide_field_id'            => '#contact-alg_eu_vat-' . wpfactory_wc_eu_vat_get_field_id() . '_customer_decide',
			'eu_vat_valid_vat_but_not_exempted_field_id' => '#contact-alg_eu_vat-' . wpfactory_wc_eu_vat_get_field_id() . '_valid_vat_but_not_exempted',
			'eu_vat_field_position_id'                   => apply_filters(
				'alg_wc_eu_vat_field_position_block_checkout',
				''
			),
			'eu_vat_get_show_in_countries'               => wpfactory_wc_eu_vat()->core->country_locale->show_in_countries,
		);
	}
	...
	public function register_euvat_block_editor_scripts() {
		...
		$script_asset_path = (
			dirname( __FILE__ ) .
			'/build/checkout-eu-vat-field-block/index.asset.php'
		);
		$script_asset      = (
			...
			array(
				'dependencies' => array(),
				'version'      => WPFACTORY_WC_EU_VAT_VERSION,
			)
		);

		wp_register_script(
			'wpfactory-wc-eu-vat-field-block-editor',
			$script_url,
	...
	public function register_euvat_block_frontend_scripts() {
		...
		if ( 'yes' === get_option( 'alg_wc_eu_vat_checkout_block_field_dependencies', 'no' ) ) {
			$script_asset['dependencies'][] = 'wpfactory-wc-eu-vat';
		}
