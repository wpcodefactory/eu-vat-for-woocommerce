...
if ( ! class_exists( 'WPFactory_WC_EU_VAT_Meta_Boxes' ) ) :

class WPFactory_WC_EU_VAT_Meta_Boxes {
	...
	function get_vat_details() {
		...
		if (
			...
			! wp_verify_nonce(
				...
				'wpfactory_wc_eu_vat_get_details'
			)
		) {
			wp_die(
				esc_html__(
		...
		$eu_vat_number = wpfactory_wc_eu_vat_parse_vat( $vat_number, $country );
		$is_valid      = wpfactory_wc_eu_vat_validate_vat( $eu_vat_number['country'], $eu_vat_number['number'] );

		if ( $is_valid ) {
			$vat_response_data = wpfactory_wc_eu_vat()->core->vat_details_data;
			$order = wc_get_order( $order_id );
			if ( $order ) {
				$order->update_meta_data( wpfactory_wc_eu_vat_get_field_id() . '_details', $vat_response_data );
				$order->save();
			}
		}
		...
		wp_safe_redirect(
			add_query_arg(
				'wpfactory_wc_eu_vat_details_result',
	...
	function admin_notice() {
		if ( ! isset( $_GET['wpfactory_wc_eu_vat_details_result'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			return;
		}
		$result  = sanitize_text_field( wp_unslash( $_GET['wpfactory_wc_eu_vat_details_result'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$message = (
			( 'success' === $result ) ?
	 ...
	 * @since   4.3.0
	 *
	 * @todo    (dev) save actual EU VAT number used on checkout (instead of `$_order->get_meta( '_' . wpfactory_wc_eu_vat_get_field_id() )`)?
	...
	function output_meta_box_data( $object, $do_return_table_data = false ) {
		...
		$_customer_ip_address = ( wpfactory_wc_eu_vat()->core->is_wc_version_below_3_0_0 ? $_order->customer_ip_address : $_order->get_customer_ip_address() );

		// Country by IP
		$customer_country = wpfactory_wc_eu_vat_get_customers_location_by_ip( $_customer_ip_address );

		// Customer EU VAT number
		if ( '' == ( $customer_eu_vat_number = $_order->get_meta( '_' . wpfactory_wc_eu_vat_get_field_id() ) ) ) {
		...
		$table_data = array(
			...
			array(
				...
				wpfactory_wc_eu_vat_get_country_name_by_code( $customer_country ) . ' [' . $customer_country . ']',
			),
			array(
		...
		// VAT Details
		$customer_eu_vat_details = $_order->get_meta( wpfactory_wc_eu_vat_get_field_id() . '_details' );
		...
		echo '<p>' .
			'<a href="' . esc_url( add_query_arg( array(
				...
				'nonce'                               => wp_create_nonce( 'wpfactory_wc_eu_vat_validate_vat' ),
			) ) ) . '">' .
		...
		echo '<p>' .
			'<a href="' . esc_url( add_query_arg( array(
				...
				'nonce'           => wp_create_nonce( 'wpfactory_wc_eu_vat_get_details' ),
			) ) ) . '">' .
	 ...
	 * @todo    (dev) Request Identifier: merge with `save_request_identifier_to_order()` (`WPFactory_WC_EU_VAT_Orders` class)
	 * @todo    (dev) `wpfactory_wc_eu_vat()->core->eu_vat_response_data`: clear after use?
	 */
	function validate_vat_and_maybe_remove_taxes() {
		...
		$preserve_countries           = wpfactory_wc_eu_vat()->core->eu_vat_ajax_instance->get_preserve_countries();
		$preserve_countries_condition = false;

		$order_id = sanitize_text_field( wp_unslash( $_GET['validate_vat_and_maybe_remove_taxes'] ) );
		if ( ! $order_id ) {
			wp_die( esc_html__( 'Invalid order ID.', 'eu-vat-for-woocommerce' ) );
		}

		$order = wc_get_order( $order_id );
		if ( ! $order ) {
			wp_die( esc_html__( 'Invalid order.', 'eu-vat-for-woocommerce' ) );
		}

		$vat_id          = $order->get_meta( '_' . wpfactory_wc_eu_vat_get_field_id() );
		$billing_company = $order->get_billing_company();

		if ( '' != $vat_id ) {

			$eu_vat_number = wpfactory_wc_eu_vat_parse_vat( $vat_id, $order->get_billing_country() );
			...
			if (
				...
				// Request Identifier
				$vat_response_data = wpfactory_wc_eu_vat()->core->eu_vat_response_data;
...
return new WPFactory_WC_EU_VAT_Meta_Boxes();
