...
if ( ! class_exists( 'WPFactory_WC_EU_VAT_WC_Report' ) ) :

class WPFactory_WC_EU_VAT_WC_Report extends WC_Admin_Report {
	...
	public function get_flag_img( $country ) {
		...
		$return = (
			file_exists( wpfactory_wc_eu_vat()->plugin_path() . $path ) ?
			'<img' .
				' title="' . wpfactory_wc_eu_vat_get_country_name_by_code( $country ) . ' (' . strtoupper( $country ) . ')"' .
				' src="' . wpfactory_wc_eu_vat()->plugin_url() . $path . '"' .
			'> ' :
			''
		);
		return $return . wpfactory_wc_eu_vat_get_country_name_by_code( $country ) . ' (' . strtoupper( $country ) . ')';
	}
