...
if ( ! class_exists( 'WPFactory_WC_EU_VAT_Admin' ) ) :

class WPFactory_WC_EU_VAT_Admin {
	...
	function __construct() {
		...
		// Meta boxes
		$this->meta_boxes = require_once plugin_dir_path( __FILE__ ) . 'class-wpfactory-wc-eu-vat-meta-boxes.php';

		// Admin order list
		require_once plugin_dir_path( __FILE__ ) . 'class-wpfactory-wc-eu-vat-admin-order-list.php';

		// Admin user list
		require_once plugin_dir_path( __FILE__ ) . 'class-wpfactory-wc-eu-vat-admin-user-list.php';

		// Exempt VAT from admin
		require_once plugin_dir_path( __FILE__ ) . 'class-wpfactory-wc-eu-vat-admin-exempt.php';

		// Advertise
		require_once plugin_dir_path( __FILE__ ) . 'class-wpfactory-wc-eu-vat-advertise.php';
	...
	function admin_inline_js() {
		...
		<script type="text/javascript">
			jQuery( function ( $ ) {
				...
				$( '#_billing_country' ).on( 'change', wpfactory_wc_eu_vat_validate_vat_admin );

				// On input, start the countdown
				admin_vat_input.on( 'input', function () {
					clearTimeout( admin_input_timer );
					admin_input_timer = setTimeout( wpfactory_wc_eu_vat_validate_vat_admin, done_input_interval );
				} );

				$( '#_billing_company' ).on( 'input', function () {
					clearTimeout( input_timer_company );
					input_timer_company = setTimeout( wpfactory_wc_eu_vat_validate_vat_admin, done_input_interval );
				} );

				/**
				 * wpfactory_wc_eu_vat_validate_vat_admin
				 *
				 * @version 4.7.0
				 * @since   1.0.0
				 */
				function wpfactory_wc_eu_vat_validate_vat_admin() {
					console.log("yo are admin");
					...
					var data = {
						...
						'eu_vat_to_check'                   : admin_vat_number_to_check,
						'billing_country': $( '#_billing_country' ).val(),
						'billing_company': $( '#_billing_company' ).val(),
						'nonce'                             : '<?php echo esc_attr( wp_create_nonce( 'wpfactory_wc_eu_vat_nonce' ) ); ?>',
					};
					$.ajax( {
						type: "POST",
	...
	function admin_order_is_vat_exempt( $is_exempt, $order ) {
		...
		if (
			...
			if ( wpfactory_wc_eu_vat()->core->check_current_user_roles( get_option( 'alg_wc_eu_vat_exempt_for_user_roles', array() ) ) ) {

				$is_exempt = true;

			} elseif ( wpfactory_wc_eu_vat()->core->check_current_user_roles( get_option( 'alg_wc_eu_vat_not_exempt_for_user_roles', array() ) ) ) {

				$is_exempt = false;

			} elseif ( wpfactory_wc_eu_vat()->core->is_validate_and_exempt() && wpfactory_wc_eu_vat()->core->is_valid_and_exists() ) {
	...
	function order_customer_decide( $order ) {
		if ( 1 != $order->get_meta( '_' . wpfactory_wc_eu_vat_get_field_id() . '_customer_decide' ) ) {
			return;
		}
	...
	function add_eu_vat_reports( $reports ) {
		...
		$reports['taxes']['reports']['wpfactory_wc_eu_vat'] = array(
	...
	function output_eu_vat_report() {
		require_once plugin_dir_path( __FILE__ ) . 'class-wpfactory-wc-eu-vat-wc-report.php';
		$report = new WPFactory_WC_EU_VAT_WC_Report();
	...
	function add_to_admin_order_display( $fields ) {
		$fields[ wpfactory_wc_eu_vat_get_field_id( true ) ] = array(
			'type'  => 'text',
			'label' => do_shortcode(
				get_option(
			...
			'show'  => ( ! in_array(
				'in_billing_address',
				wpfactory_wc_eu_vat()->core->display->get_positions()
			) ),
		);
		return $fields;
	}
	...
	function add_to_ajax_get_customer_details( $data, $customer, $user_id ) {
		$id  = wpfactory_wc_eu_vat_get_field_id( true );
		$key = wpfactory_wc_eu_vat_get_field_id();
...
return new WPFactory_WC_EU_VAT_Admin();
