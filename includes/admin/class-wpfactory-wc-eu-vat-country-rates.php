...
if ( ! class_exists( 'WPFactory_WC_EU_VAT_Countries_VAT_Rates_Tool' ) ) :

class WPFactory_WC_EU_VAT_Countries_VAT_Rates_Tool {
	...
	function add_eu_countries_vat_rates() {
		...
		if ( ! isset( $_POST['wpfactory_add_eu_countries_vat_rates'] ) ) {
			return;
		}

		if (
			! isset( $_POST['wpfactory_wc_eu_vat_nonce'] ) ||
			! check_admin_referer( 'wpfactory_wc_eu_vat_rates', 'wpfactory_wc_eu_vat_nonce' )
		) {
			return;
		}
		...
		foreach ( $this->get_european_union_countries_with_vat() as $country => $rate ) {
			...
			$tax_rate    = array(
				...
				'tax_rate'          => $rate,
				'tax_rate_name'     => (
					isset( $_POST['wpfactory_wc_eu_vat_tax_name'] ) ?
					sanitize_text_field( wp_unslash( $_POST['wpfactory_wc_eu_vat_tax_name'] ) ) :
	...
	function add_eu_countries_vat_rates_tool() {
		add_submenu_page(
			...
			__( 'EU country VAT Rates', 'eu-vat-for-woocommerce' ),
			'manage_woocommerce',
			'wpfactory-wc-eu-vat-country-rates',
	...
	function create_eu_countries_vat_rates_tool() {
		...
		'</h3>';
		$header_html .= '<p><em>' .
			'<a href="' . esc_url( admin_url( 'admin.php?page=wc-settings&tab=wpfactory_wc_eu_vat' ) ) . '">' .
		...
		$the_name      = (
			isset( $_POST['wpfactory_wc_eu_vat_tax_name'] ) ? // phpcs:ignore WordPress.Security.NonceVerification.Missing
			sanitize_text_field( wp_unslash( $_POST['wpfactory_wc_eu_vat_tax_name'] ) ) : // phpcs:ignore WordPress.Security.NonceVerification.Missing
			__( 'VAT', 'eu-vat-for-woocommerce' )
		);

		$nonce_field   = wp_nonce_field( 'wpfactory_wc_eu_vat_rates', 'wpfactory_wc_eu_vat_nonce', true, false );
		$data[]        = array(
			__( 'Name', 'eu-vat-for-woocommerce' ) . '<br>' .
			'<input class="input-text" type="text" name="wpfactory_wc_eu_vat_tax_name" value="' . esc_attr( $the_name ) . '">',
			'<em>' . __( 'Note: will not add duplicates for country.', 'eu-vat-for-woocommerce' ) . '</em>' . '<br>' .
			$nonce_field .
			'<input class="button-primary" type="submit" name="wpfactory_add_eu_countries_vat_rates" value="' . __( 'Add EU country VAT Rates', 'eu-vat-for-woocommerce' ) . '">',
		);
		$the_tool_html .= '<p>';
		$the_tool_html .= '<form method="post" action="">';
		$the_tool_html .= wpfactory_wc_eu_vat_get_table_html( $data, array(
		...
		foreach ( $eu_vat_rates as $country => $rate ) {
			$data[] = array(
				$i++,
				$country . ' - ' . wpfactory_wc_eu_vat_get_country_name_by_code( $country ),
				$rate . '%'
			);
		}
		$the_tool_html .= wpfactory_wc_eu_vat_get_table_html( $data, array(
		...
		foreach ( $standard_tax_rates as $tax_rate_object ) {
			$data[] = array(
				$i++,
				$tax_rate_object->tax_rate_country . ' - ' . wpfactory_wc_eu_vat_get_country_name_by_code( $tax_rate_object->tax_rate_country ),
				$tax_rate_object->tax_rate . '%',
				$tax_rate_object->tax_rate_name
			);
		}
		$the_tool_html .= wpfactory_wc_eu_vat_get_table_html( $data, array(
			'table_class' => 'widefat',
		...
		echo '<div class="wrap">' .
			wp_kses( $the_tool_html, wpfactory_wc_eu_vat()->core->get_allowed_form_html() ) .
		'</div>';
	}
...
return new WPFactory_WC_EU_VAT_Countries_VAT_Rates_Tool();
