...
if ( ! class_exists( 'WPFactory_WC_EU_VAT_Admin_Order_List' ) ) :

class WPFactory_WC_EU_VAT_Admin_Order_List {
	...
	function add_order_columns( $columns ) {
		$columns['wpfactory_wc_eu_vat'] = __( 'EU VAT', 'eu-vat-for-woocommerce' );
		return $columns;
	}
	...
	function render_order_columns( $column ) {
		if ( 'wpfactory_wc_eu_vat' === $column ) {
			echo esc_html( get_post_meta( get_the_ID(), '_'. wpfactory_wc_eu_vat_get_field_id(), true ) );
	...
	function render_wc_order_columns( $column, $order ) {
		if ( 'wpfactory_wc_eu_vat' === $column ) {
			$key           = '_'. wpfactory_wc_eu_vat_get_field_id();
...
return new WPFactory_WC_EU_VAT_Admin_Order_List();
