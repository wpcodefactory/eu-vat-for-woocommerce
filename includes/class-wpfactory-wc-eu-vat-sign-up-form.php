...
if ( ! class_exists( 'WPFactory_WC_EU_VAT_Sign_Up_Form' ) ) :

class WPFactory_WC_EU_VAT_Sign_Up_Form {
	...
	function validate_field_in_woocommerce_register_form( $username, $email, $errors ) {
		...
		if (
			...
			$is_valid = wpfactory_wc_eu_vat()->core->check_and_save_eu_vat(
				$eu_vat_to_check,
				$form_country,
				$form_company_name
			);
...
return new WPFactory_WC_EU_VAT_Sign_Up_Form();
