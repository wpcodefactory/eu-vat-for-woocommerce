...
if ( ! class_exists( 'WPFactory_WC_EU_VAT_AJAX' ) ) :

class WPFactory_WC_EU_VAT_AJAX {
	...
	function __construct() {
		...
		add_action(
			'wp_ajax_' . 'wpfactory_wc_eu_vat_validate_action',
			array( $this, 'ajax_vat_validate' )
		);
		add_action(
			'wp_ajax_nopriv_' .  'wpfactory_wc_eu_vat_validate_action',
			array( $this, 'ajax_vat_validate' )
		);
	}
	...
	function enqueue_scripts() {
		...
		wp_enqueue_script(
			'wpfactory-wc-eu-vat',
			wpfactory_wc_eu_vat()->plugin_asset_url( '/js/wpfactory-wc-eu-vat.js' ),
			array( 'jquery' ),
		...
		wp_localize_script(
			'wpfactory-wc-eu-vat',
			'wpfactory_wc_eu_vat_ajax_object',
			array(
	 ...
	 */
	function ajax_vat_validate() {

		check_ajax_referer( 'wpfactory_wc_eu_vat_nonce', 'nonce' );

		$post       = wp_unslash( $_POST );
		$vat_number = sanitize_text_field( $post['eu_vat_to_check'] ?? '' );
		...
		if (
			...
			$vat_number = WC()->customer->get_meta(
				wpfactory_wc_eu_vat()->core->checkout_block->get_block_field_id()
			);
		}
		...
		$vat_customer_decide        = sanitize_text_field( $post['eu_vat_customer_decide'] ?? false );
		$vat_valid_but_not_exempted = sanitize_text_field( $post['eu_vat_valid_vat_but_not_exempted'] ?? false );
...
return new WPFactory_WC_EU_VAT_AJAX();
