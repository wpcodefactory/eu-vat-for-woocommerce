...
if ( ! class_exists( 'WPFactory_WC_EU_VAT_Compatibility' ) ) :

class WPFactory_WC_EU_VAT_Compatibility {
	...
	function wpo_wcpdf_extend_after_billing_address( $type, $pdf_order ) {
		if (
			function_exists( 'wpfactory_wc_eu_vat_get_field_id' ) &&
			! in_array(
				'in_billing_address',
				wpfactory_wc_eu_vat()->core->display->get_positions()
			)
		) {
			$vat_id = $pdf_order->get_meta( '_' . wpfactory_wc_eu_vat_get_field_id() );
...
return new WPFactory_WC_EU_VAT_Compatibility();
