...
if ( ! class_exists( 'WPFactory_WC_EU_VAT_Settings_Admin' ) ) :

class WPFactory_WC_EU_VAT_Settings_Admin extends WPFactory_WC_EU_VAT_Settings_Section {
				...
				'desc'     => '<ul style="background-color:white;padding:10px 30px;color:black;list-style-type:square;margin-top:1em;">' .
					'<li>' .
						sprintf(
							...
							'<code>' . '_' . wpfactory_wc_eu_vat_get_field_id() . '</code>'
						) .
					'</li>' .
					'<li>' .
						sprintf(
							...
							'<a href="' . admin_url( 'tools.php?page=wpfactory-wc-eu-vat-country-rates' ) . '">' .
					...
					'<li>' .
						sprintf(
							...
							'<a href="' . admin_url( 'admin.php?page=wc-reports&tab=taxes&report=wpfactory_wc_eu_vat' ) . '">' .
...
return new WPFactory_WC_EU_VAT_Settings_Admin();
