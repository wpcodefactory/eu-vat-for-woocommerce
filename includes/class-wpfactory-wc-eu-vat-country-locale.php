...
if ( ! class_exists( 'WPFactory_WC_EU_VAT_Country_Locale' ) ) :

class WPFactory_WC_EU_VAT_Country_Locale {
	...
	function set_eu_vat_country_locale( $country_locales ) {
		...
		if (
			...
			foreach ( $country_locales as $country_code => &$country_locale ) {
				...
				$country_locale[ wpfactory_wc_eu_vat_get_field_id( true ) ] = array(
					'required' => $is_required,
					'hidden'   => $hidden,
				);

				$country_locale[ wpfactory_wc_eu_vat_get_field_id( true ) . '_valid_vat_but_not_exempted' ] = array(
					'hidden' => $hidden,
				);
			}
			...
			if ( ! empty( $show_eu_vat_field_countries[0] ) ) {
				foreach ( $show_eu_vat_field_countries as $count_code ) {
					$country_locales[ $count_code ][ wpfactory_wc_eu_vat_get_field_id( true ) ] = array(
						'hidden' => false,
					);

					$country_locales[ $count_code ][ wpfactory_wc_eu_vat_get_field_id( true ) . '_valid_vat_but_not_exempted' ] = array(
						'hidden' => false,
					);
				}
			}
			...
			if ( ! empty( $arr_dif ) ) {
				foreach ( $arr_dif as $con ) {
					...
					$country_locales[ $con ][ wpfactory_wc_eu_vat_get_field_id( true ) ] = array(
						'hidden'   => $hidden,
						'required' => $is_required,
					);

					$country_locales[ $con ][ wpfactory_wc_eu_vat_get_field_id( true ) . '_valid_vat_but_not_exempted' ] = array(
						'hidden' => $hidden,
					);
				}
			}
			...
			if ( ! empty( $required_eu_vat_field_countries ) ) {
				foreach ( $required_eu_vat_field_countries as $country_code_re ) {
					...
					$country_locales[ $country_code_re ][ wpfactory_wc_eu_vat_get_field_id( true ) ] = array(
						'required' => $is_required,
						'hidden'   => $hidden,
					);

					$country_locales[ $country_code_re ][ wpfactory_wc_eu_vat_get_field_id( true ) . '_valid_vat_but_not_exempted' ] = array(
						'hidden' => $hidden,
					);
				}
			}
	...
	function set_eu_vat_country_locale_field_selectors( $locale_fields ) {
		$locale_fields[ wpfactory_wc_eu_vat_get_field_id( true ) ] = '#' . wpfactory_wc_eu_vat_get_field_id() . '_field';
		$locale_fields[ wpfactory_wc_eu_vat_get_field_id( true ) . '_valid_vat_but_not_exempted' ] = '#' . wpfactory_wc_eu_vat_get_field_id() . '_valid_vat_but_not_exempted_field';
	...
	function set_eu_vat_country_locale_default( $default_locale ) {
		...
		$default_locale[ wpfactory_wc_eu_vat_get_field_id( true ) ] = array(
			'required' => $required,
			'hidden'   => false,
		);

		$default_locale[ wpfactory_wc_eu_vat_get_field_id( true ) . '_valid_vat_but_not_exempted' ] = array(
			'hidden' => false,
		);
...
return new WPFactory_WC_EU_VAT_Country_Locale();
