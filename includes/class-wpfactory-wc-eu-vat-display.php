...
if ( ! class_exists( 'WPFactory_WC_EU_VAT_Display' ) ) :

class WPFactory_WC_EU_VAT_Display {
	...
	function add_eu_vat_number_to_order_display( $order ) {
		...
		$order_id                    = wpfactory_wc_eu_vat_get_order_id( $order );
		$html                        = '';
		$option_name                 = '_' . wpfactory_wc_eu_vat_get_field_id();
		$option_name_customer_decide = '_' . wpfactory_wc_eu_vat_get_field_id() . '_customer_decide';
	...
	function add_eu_vat_number_to_order_billing_address( $fields, $_order ) {
		$field_name  = wpfactory_wc_eu_vat_get_field_id();
	...
	function add_eu_vat_number_to_address_formats( $address_formats ) {
		if ( wpfactory_wc_eu_vat_is_block_checkout() ) {
	...
	function add_eu_vat_number_to_editable_fields( $address, $load_address ) {
		if ( 'billing' === $load_address ) {
			...
			$address[ $field_id ] = wpfactory_wc_eu_vat()->core->get_field_data();
	...
	function save_eu_vat_number_from_editable_fields( $user_id, $load_address ) {
		if ( 'billing' === $load_address ) {
			...
			$field_id    = wpfactory_wc_eu_vat_get_field_id();
			$field_id_cd = wpfactory_wc_eu_vat_get_field_id() . '_customer_decide';
...
return new WPFactory_WC_EU_VAT_Display();
